...
<style>
@import "https://germanfrelo.github.io/base-css-stylesheet/base.css" layer
	(base);

@import "https://codepen.io/germanfrelo/pen/mdMYKza.css" layer
	(styles);
...
<form action="modifyQnaForm.do" name="qnaForm">
	<input type="hidden" name="qno" value="${qno.qno }"> <input
		type="hidden" name="mid" value="${loginId }"> <input
		type="hidden" name="pass" value="${qno.qpass }">
	...
	<table border="1" class="table">
		...
		<tr>
			...
			<td colspan="4"><textarea rows="5" cols="40"
					class="form-control">${qno.qcontent }</textarea></td>
		</tr>
		...
		<tr>
			<td colspan="4" align="center"><c:choose>
					<c:when
						test="${(!empty loginId && loginId == qno.qid) || responsibility == ('admin') }">
...
<ul id="replylist">
	<li style="display: none" id="template"><span> 관리자 </span><b>관리자의
			댓글 내용</b>
		<button id='delBtn'>삭제</button></li>
</ul>


<button type="button" onclick="location.href='qnaList.do'">⏮QnA
	게시판으로 돌아가기</button>
...
let page = 1;
	...
	//result에 있는 list 한건 한건해서 목록 그려주는거
...
//등록버튼에 대한 이벤트! 
	document.querySelector('#addReply').addEventListener('click', function(e){
		...
		.then(result => {
			if(result.retCode=="OK"){
				document.querySelector('input[id=rpcontent]').value = '';
				showList(-1); 
			}else{
				alert('ERROR');
			}
